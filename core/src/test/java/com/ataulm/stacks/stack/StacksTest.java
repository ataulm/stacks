...
public class StacksTest {
    ...
    public void given_stacksWithMultipleLevels_when_EligibleParentsForAGivenStackIsRequested_then_ListDoesNotContainThatStackOrDescendantsOfThatStack() {
        Stack a = Stack.create("a", "a", Optional.<String>absent());
        Stack a1 = Stack.create("a.1", "a.1", Optional.of(a.id()));
        Stack a2 = Stack.create("a.2", "a.2", Optional.of(a.id()));

        Stack b = Stack.create("b", "b", Optional.<String>absent());
        Stack b1 = Stack.create("b.1", "b.1", Optional.of(b.id()));
        Stack b1dot1 = Stack.create("b.1.1", "b.1.1", Optional.of(b1.id()));

        Stack c = Stack.create("c", "c", Optional.<String>absent());
        Stack c1 = Stack.create("c.1", "c.1", Optional.of(c.id()));
        Stack c2 = Stack.create("c.2", "c.2", Optional.of(c.id()));
